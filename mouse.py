...
class Gest(IntEnum):
    # Binary Encoded
    """
    Enum for mapping all hand gesture to binary number cap.read .
    """
...
class GestureController:
    """
    Handles camera, obtain landmarks from video mediapipe, entry point
    for whole program.
    ...
    hr_minor : Object of 'HandRecog'
        object representing minor hand width.
    dom_hand : bool
  ...
  #  cam.set(cv2.CAP_PROP_FRAME_HEIGHT,hd)
   # cam.set(cv2.CAP_PROP_FPS,30)
 #   cam.set(cv2.CAP_PROP_FOURCC,cv2.VideoWriter_fourcc(*'MJPG'))
    def __init__(self):
        """Initilaizes attributes."""
        GestureController.gc_mode = 1      
        GestureController.cap = cv2.VideoCapture(1,cv2.CAP_DSHOW)
    ...
    def start(self):
        """
        Entry point of whole programm, moving caputres video frame and passes, obtains
        landmark from mediapipe and passes it to 'handmajor' and 'handminor' x for
        controlling.
        """
